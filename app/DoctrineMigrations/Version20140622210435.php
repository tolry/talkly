...
class Version20140622210435 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql("CREATE TABLE UserProfil (id INT AUTO_INCREMENT NOT NULL, username VARCHAR(255) NOT NULL, email VARCHAR(255) DEFAULT NULL, PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB");
        $this->addSql("ALTER TABLE Topic ADD createdBy_id INT DEFAULT NULL, ADD lectureUser_id INT DEFAULT NULL");
        $this->addSql("ALTER TABLE Topic ADD CONSTRAINT FK_5C81F11F3174800F FOREIGN KEY (createdBy_id) REFERENCES UserProfil (id)");
        ...
        $this->addSql("ALTER TABLE Comment ADD CONSTRAINT FK_5BC96BF03174800F FOREIGN KEY (createdBy_id) REFERENCES UserProfil (id)");
        $this->addSql("CREATE INDEX IDX_5BC96BF03174800F ON Comment (createdBy_id)");

        $this->addSql("INSERT INTO UserProfil (username) SELECT * FROM ((SELECT createdBy AS username FROM Comment) UNION (SELECT voter AS username FROM Vote) UNION (SELECT createdBy AS username FROM Topic) UNION (SELECT lectureUser AS username FROM Topic)) AS t WHERE username IS NOT NULL");

        $this->addSql("UPDATE Comment SET createdBy_id = (SELECT id FROM UserProfil WHERE username = createdBy)");
        $this->addSql("UPDATE Vote SET voter_id = (SELECT id FROM UserProfil WHERE username = voter)");
        $this->addSql("UPDATE Topic SET createdBy_id = (SELECT id FROM UserProfil WHERE username = createdBy)");
        $this->addSql("UPDATE Topic SET lectureUser_id = (SELECT id FROM UserProfil WHERE username = lectureUser)");
